...
export const ProductModal = ({
  ...
  return (
    <>
      <PathModalXl />
      <button
        className="absolute bottom-0 right-0 z-10 flex h-20 w-20 items-center justify-center rounded-full bg-primary duration-300 hover:bg-bg_secondary mdOnly:h-12 mdOnly:w-12"
      ...
      {isOpen && (
        ...
        >
          <div className="relative h-full xl:h-auto">
            <button
              onClick={() => setMenuClosed()}
              className="fixed right-2 top-1 z-10 rounded-full bg-primary p-4 duration-300 hover:bg-bg_secondary xl:absolute"
            >
              <BurgerCross className="h-2 w-2 text-white" />
            </button>
            <div
              className="productModal relative h-full overflow-y-auto bg-bg_secondary xl:flex xl:h-[580px] xl:w-[1000px] xl:flex-row"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="xl:flex xl:h-full xl:flex-col">
                <div className="relative h-full md:h-96 xl:h-full xl:w-[550px] xl:flex-shrink-0">
                  <Image
                    quality={80}
                    fill
              ...
              <div className="px-4 py-3 md:px-3 md:py-10 xl:px-10">
                ...
                <div className="mb-4 flex gap-2 md:flex-wrap md:gap-3 smOnly:flex-col">
                  {product?.advantage1 && (
                    <p className="max-w-max rounded-r-full bg-primary_T/20 px-3 py-2 text-t10 text-primary md:text-t12">
                      {product.advantage1}
                    </p>
                  )}
                  {product?.advantage2 && (
                ...
                <div className="text-t14 flex flex-row text-center font-bold md:text-t18">
                  <button
                    className={cn(
                      "w-full border-solid pb-1 pt-2 text-primary",
