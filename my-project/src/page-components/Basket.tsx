...
const Basket = ({ data, lang }: { data: any; lang: any }) => {
    ...
    return (
        <section className='container py-40 xl:flex justify-between'>
            <div className="w-full xl:w-[750px]">
                <h1 className='text-t32 tracking-wider mb-10 font-bold'>
                    {data.basket.heading}
                </h1>
                        ...
                        {tovar.map((item: any) => (
                            <tbody key={item.id} className="smOnly:hidden">
                                ...
                                <tr className="border-b">
                                    <td className="flex ">
                                        <div className="relative w-[71px] h-[80px] xl:flex-shrink-0 mr-4">
                                            <Image
                                                quality={80}
                                                fill
                                                src={item.photo}
                                        ...
                                        <div><p className="py-2  mdOnly:text-t16 xl:text-t18 ">{item.productName.replace(/#/g, '')}</p>
                                            <p className="text-t14 text-dark/60"> {item.capacity}</p></div> </td>
                                    <td className="py-2 leading-5 text-[#333333] mdOnly:text-t16 xl:text-t18 text-c=left ">
                                        {(lang === en
                            ...
                            <tbody key={item.id} className="md:hidden">
                                ...
                                <tr className="border-b">
                                    <td className="flex">
                                        ...
                                        <div><p className="py-2 text-t16 ">{item.productName.replace(/#/g, '')}</p>
                                            <p className="text-t14 text-dark/60 mb-1"> {item.capacity}</p>
                                            <div className="flex justify-evenly">
                                                <div className="border-solid border-2 border-text-[#33333399]">
                                                    <button
                                                        onClick={() => handleQuantityChange(item.id, -1)}
                                    ...
                                    <td className="py-2 leading-5 text-[#333333]">
                                        <ul className="text-left">
                                            <li className="mb-4 text-t16">  {(lang === en
                                                ? state && state.products.find((items: any) => items.id === item.id)
                                                    ? convertPrice(
                                                        state.products.find((items: any) => items.id === item.id)!.price,
                                                        state.currencies.find((currency: any) => currency.id === "EUR")?.rate || 1
                ...
                <div className="text-end mb-8">
                    ...
                    <button className="ml-auto mb-6 text-black w-[225px] text-t14 xl:text-t16 flex justify-between" onClick={handleToggleInput}> {lang === en ? 'Add promo code' : 'Додати промокод '}   <BurgerCross className="w-4 h-4 origin-center rotate-45" /></button>
                    ...
                    {isInputOpen && (
                        <div className="relative">
                            <input
                                ...
                                className={`text-t14 xl:text-t16   p-2 border-gray-300 w-[225px] }`}
                            />
                            <button
                ...
                <div className="ml-auto mb-8 flex w-52 justify-between text-t14 xl:text-t16 " >
                    <p>
                ...
                <div className="ml-auto flex w-52 justify-between text-t14 xl:text-t16 6 mb-10">
                    <p className="text-black opacity-60">
                        {data.basket.total}</p> <p className="text-t16 font-bold">{lang === en ? convertPrice(totalPrice, state.currencies.find((currency: any) => currency.id === "EUR")?.rate || 1) : totalPrice} {lang === en ? '€' : '₴'}
                    </p>

                </div>

                <div className="justify-end  flex mb-[66px] xl:mb-28">
                    <button
                        onClick={handleOrderClick}
                        className={`bg-black px-6 py-4 smOnly:w-full text-white rounded text-t16 xl:text-t18 ml-auto ${isButtonDisabled ? 'opacity-50 cursor-not-allowed' : ''} `}
                    >
                        {data.basket.toOrder}
                    </button>
                </div>
